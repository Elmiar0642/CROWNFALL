...
const char *cf_get_state_json(CfGame *game) {
    ...
    n += snprintf(json + n, sizeof(json) - (size_t)n,
                  "{\"session_id\":\"%s\",\"turn_id\":%d,\"team_count\":%d,\"current_player\":%d,\"pieces\":[",
    ...
    for (i = 0; i < game->piece_count && n < (int)sizeof(json) - 128; i++) {
        ...
        if (!p->alive) continue;
        n += snprintf(json + n, sizeof(json) - (size_t)n,
                      "%s{\"id\":%d,\"team\":%d,\"label\":\"%s\",\"layer\":%d,\"x\":%d,\"y\":%d}",
                      n && json[n - 1] != '[' ? "," : "", p->id, p->team, p->label, p->pos.layer, p->pos.x, p->pos.y);
    }
